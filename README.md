# [SAP Fiori Capability] SAPUI5 Basic Training
## DAY 5 - JSON Binding Activity Output

### Data Binding Basics Panel with Simple Form
![Image of the Data Binding Basics Panel](./images/sapui5_basic_day5_1_data_bind_panel.png)

### Address Details Panel with Simple Form
![Image of the Address Details Panel](./images/sapui5_basic_day5_2_address_panel.png)

* Unchecking the Enabled checkbox will make those fields with enabled property become disabled.
    ![Image of Enabled checkbox unselected in Data Binding Basics Panel](./images/sapui5_basic_day5_5_enabled_uncheck.png)
    *EID and Sales Amount input field are disables since they used the enabled property.*

<br> 

* Clicking the Send Email in the Address Details Panel will open a draft email in Outlook
    ![Image of the Send Email link opening an email draft window](./images/sapui5_basic_day5_6_send_email.png)
    *Send Email link opens an email draft with specified Email recipient, subject, and body.*


---


### Aggregation Binding Panel with List View
![Image of the Aggregation Binding Panel](./images/sapui5_basic_day5_3_aggreg_panel.png)

### Product Details Panel with Simple Form
![Image of the Product Details Panel](./images/sapui5_basic_day5_4_productDet_panel.png)

* Product Details Panel displays the details of the selected item in the List View
    ![Image of Product Details Panel showing the details of selected item](./images/sapui5_basic_day5_7_display_selected_item_details.png)
    *Product Details panel displays the details of product with Product ID = 'S004' as it is selected in the List View.*